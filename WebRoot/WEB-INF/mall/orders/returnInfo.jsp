					...
					<div class="col-md-9">
						...
						<div class="account_grid">
							...
							<div class="form-actions">
								<button type="button" class="btn btn-info" onclick="agree(${orders.id})">同意</button>
							<button type="button" class="btn btn-danger" onclick="refuse(${orders.id})">拒绝</button>
...
<script type="text/javascript">
	function agree(id){
		$.post("<%=path%>/orders/agree.action",{'orders.id':'${orders.id}'},function(data){
			if(data.message=="success"){
				alert("处理成功");
				location.href="<%=path%>/orders/myReturn.action";
				return;
			}else{
				alert("处理失败")
				return;
	...
	function refuse(id){
		$.post("<%=path%>/orders/refuse.action",{'orders.id':'${orders.id}'},function(data){
			if(data.message=="success"){
				alert("处理成功");
				location.href="<%=path%>/orders/myReturn.action";
				return;
