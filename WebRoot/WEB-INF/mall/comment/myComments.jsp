				...
				<tbody>
					<c:forEach items="${myComments}" var="comment">
						<tr>
							...
							<td><a href="commentInfo.action?comment.id=${comment.id}"
								class="btn btn-primary"> 查看</a>
								<button onclick="del(${comment.id})"
		...
		function del(delid) {
			art.dialog({
			    ...
			    ok: function(){	
					...
					$.post(delurl,{"comment.id":delid},function(data){
						if(data.message=="success"){							
							art.dialog({
							    ...
							    icon: 'succeed',			   
							    ok: function(){		       				       
							    	location.href="<%=path%>/comment/myComments.action";
									return;
							    }
