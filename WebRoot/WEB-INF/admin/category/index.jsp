...
<body>
	...
	<div class="content">
		...
		<div class="container-fluid">
			<div class="btn-toolbar">
				<a href="<%=path%>/category/toInsert.action" role="button"
					...
					<tbody>
						<c:forEach items="${categoryList}" var="category">
							<tr>
								<td>${category.id}</td>
								<td>${category.name}</td>
								<td>${category.descb}</td>								
								<td><a href="info.action?category.id=${category.id}"
									class="btn btn-primary"> 查看</a> <a
									href="toUpdate.action?category.id=${category.id}"
									class="btn btn-primary"> 编辑</a>
									<button onclick="del(${category.id},${page.pageNo})"
	...
	<script type="text/javascript">
		...
		function delHotel(delid,pageNo){
			...
			$.post(delurl,{"category.id":delid,pageNo:pageNo},function(data){
				if(data.message=="success"){
					alert("删除成功！");
					location.href="<%=path%>/category/list.action";
					return;
				}
